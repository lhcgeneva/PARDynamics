function [DomainSize, MemCirc] = calc_domain_size(filename, showGraphs,...
                                                  savePic, botOffset, topOffset)
...
%% Max intensity projection perpendicular to membrane, smooth, register by min/max
...
ProjRegSmooth = cellfun(@(x) Segmentation.smooth_periodic(x, smooth_window), ...
                    ProjectionRegistered, 'UniformOutput', false);
                
%% Smooth registered stack, fit left and right half from maximum
[~, maxRegSmoothInd] = cellfun(@(x) max(x),ProjRegSmooth, ...
                        'UniformOutput', false);
...
parfor k = 1: length(info)
    ...
    l = ProjRegSmooth{k}(1:maxRegSmoothInd(k) + top);
    try 
...
for k = 1 : length(info)
    ...
    try
        left_ind(k) = draw_from_here(Fl_c(k)+maxSmoothInd{k} - ...
                      maxRegSmoothInd(k)+indivArrayWidth(k));

    catch
        left_ind(k) = nan;
    end
    try
        right_ind(k)= draw_from_here(maxSmoothInd{k} - maxRegSmoothInd(k) +...
                      indivArrayWidth(k) - Fr_c(k));
    catch
        right_ind(k) = nan;
    end
end
...
%% Plot
% Plot kymograph with domain boundaries
imshow(reshape(permute(imageStack, [1,3,2]),[], width), [])
for k = 1 : length(info)
    if maxSmoothInd{k} < 500
        line([right_ind(k),right_ind(k)], [(k-1) * height + 1, k * height],...
             'Color','r','LineWidth', 2);
        line([left_ind(k),left_ind(k)], [(k-1) * height + 1, k * height],...
             'Color','r','LineWidth', 2);
...
figure; hold on;
% Plot domain size over time
subplot(2,1,1)
hold on;
for i = 1:length(info)
    plot(ProjectionSmooth{i}, 'r');
    plot(ProjRegSmooth{i}, 'b');
    legend('Projection', 'Registered projection');
end
% Plot projection across membrane, registered and unregistered
subplot(2,1,2)
